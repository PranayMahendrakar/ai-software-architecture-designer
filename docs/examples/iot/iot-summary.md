# IoT - Architecture Summary

> Design an IoT platform that ingests sensor data from thousands of devices, stores time-series metrics, triggers alerts, and provides a dashboard. The system must handle distributed, large-scale streaming data.
...
- **core-service** :8002 — Main business logic for IoT
...
## Databases
- **postgres-db** (PostgreSQL) — Used by: user-service, core-service
- **redis-cache** (Redis) — Used by: api-gateway, session management
